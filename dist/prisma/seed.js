...
async function createExercises() {
    ...
    for (const exerciseData of exercisesData) {
        const exercise = await prisma.exercise.create({
            data: { ...exerciseData, category: exerciseData.category },
        });
        exercises.push(exercise);
    }
    return exercises;
}
async function createWorkoutPlans(exercises) {
    ...
    const workoutPlans = [];
    const findExercise = (slug) => exercises.find(e => e.slug === slug);
    for (const planData of workoutPlansData) {
        const workoutPlan = await prisma.workoutPlan.create({
            data: { ...planData, level: planData.level },
        });
